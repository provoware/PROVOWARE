...
SCRIPT_ORDER = [
    ...
    "js/validation-engine.js", "js/report-generator.js", "js/ui/app-ui.js", "js/app.js",
    "tests/smoke/browser-smoke.js"
]
...
def embedded_html(label: str, project_id: str):
    ...
    bootstrap = (
        "window.__PROVOWARE_SMOKE_EMBEDDED__ = true;"
...
async def assert_result(page, label: str, width: int, height: int):
    await page.wait_for_selector('body[data-smoke-status]', timeout=20_000)
    status = await page.get_attribute("body", "data-smoke-status")
    result = await page.locator("#smoke-result").inner_text()
    if status != "passed":
        raise AssertionError(f"{label}: {result}")
    print(f"[OK] Browser-Smoke {label}: {width}x{height}\n{result}")
...
async def run_embedded(browser_type, executable_path, label: str, width: int, height: int):
    ...
    project_id = f"smoke-{label}-{uuid.uuid4().hex[:10]}"
...
async def run_real(browser_type, executable_path, base_url: str, label: str, width: int, height: int):
    ...
    try:
        await page.goto(
            f"{base_url}/?smoke=1&viewport={label}&project={project_id}",
...
async def async_main(base_url: str):
    ...
    async with async_playwright() as playwright:
        for label, width, height in (("desktop", 1440, 1000), ("mobile", 390, 844)):
            ...
            if not real:
                print(f"[HINWEIS] {label}: lokale Navigation administrativ blockiert; eingebetteter UI-Fallback wird geprüft.")
