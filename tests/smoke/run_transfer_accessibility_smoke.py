...
SCRIPT_ORDER = [
    ...
    "js/app.js", "tests/smoke/transfer-accessibility-smoke.js"
]
STYLE_ORDER = [
...
def embedded_html(label: str, project_id: str):
    ...
    bootstrap = (
        "window.__PROVOWARE_TRANSFER_SMOKE_EMBEDDED__ = true;"
...
async def assert_result(page, label: str, width: int, height: int):
    await page.wait_for_selector('body[data-transfer-smoke-status]', timeout=30_000)
    status = await page.get_attribute("body", "data-transfer-smoke-status")
    result = await page.locator("#transfer-smoke-result").inner_text()
    if status != "passed":
        raise AssertionError(f"{label}: {result}")
    print(f"[OK] Transfer-/A11y-Smoke {label}: {width}x{height}\n{result}")
...
async def run_embedded(browser_type, executable_path, label: str, width: int, height: int):
    ...
    project_id = f"transfer-smoke-{label}-{uuid.uuid4().hex[:10]}"
...
async def run_real(browser_type, executable_path, base_url: str, label: str, width: int, height: int):
    ...
    try:
        ...
        await page.add_script_tag(content=(ROOT / "tests" / "smoke" / "transfer-accessibility-smoke.js").read_text(encoding="utf-8"))
        await assert_result(page, label, width, height)
        return True
...
async def async_main(base_url: str):
    ...
    async with async_playwright() as playwright:
        for label, width, height in (("desktop", 1440, 1000), ("mobile", 390, 844)):
            ...
            if not real:
                print(f"[HINWEIS] {label}: lokale Navigation administrativ blockiert; eingebetteter Transfer-/A11y-Fallback wird geprüft.")
