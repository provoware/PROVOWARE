...
def pruefe_iterationsfolge(status: dict[str, Any], baseline: dict[str, Any], fehler: list[str]) -> None:
    ...
    if isinstance(letzte, str) and isinstance(naechste, str):
        ...
        elif int(match_naechste.group(1)) != int(match_letzte.group(1)) + 1:
...
def pruefe() -> list[str]:
    ...
    baseline = als_objekt(lade_json(ROOT / "CURRENT_BASELINE.json"), "CURRENT_BASELINE.json", fehler)
    ...
    elif masterplan.get("sha256") != baseline.get("masterplan_sha256"):
